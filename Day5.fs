...
let samePolymer (a: char) (b: char) = Char.Equals(Char.ToLower a, Char.ToLower b)
let polymersWillReact (a: char) (b: char) = samePolymer a b && not (Char.Equals(a, b))

let isNotNull a = match a with | null -> false | _ -> true

let calcNumLeftInReaction (text: string) =
...
let trialRemovingPolymer (text: string) (c: char) =
